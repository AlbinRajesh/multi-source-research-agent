...
import { uploadFile, deleteDocument, listDocuments, getUploadStatus } from "../api/upload";
...
export default function FileUploader({ docs, selectedDocId, onSelectDoc, setDocs }) {
    ...
    try {
      ...
      while (true) {
        ...
        if (status.status === "ready") {
          setDocs((prev) => [
            ...prev,
            { doc_id: docId, filename: status.filename, chunks_indexed: status.chunks_indexed },
          ]);
          break;
        }
  ...
  return (
    <aside className="w-[340px] shrink-0 flex flex-col bg-surface-container-low border-r border-outline-variant/30 overflow-y-auto">
      <div className="p-lg flex flex-col gap-xl">
        ...
        <label
          className={`group relative p-lg rounded-xl bg-surface-container-lowest border-2 border-dashed transition-all cursor-pointer flex flex-col items-center gap-md text-center ${
        ...
        {error && (
          <div className="px-md py-sm rounded-lg bg-error-container text-on-error-container font-label-sm">
            {error}
          </div>
        )}
              ...
              {docs.map((d) => {
                ...
                return (
                  <div
                    ...
                    className={`group flex items-center p-md rounded-lg transition-all shadow-sm cursor-pointer ${
                      isSelected
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded flex items-center justify-center mr-md ${style.bg} ${style.fg}`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {isSelected && (
                          <span className="rounded-full border border-primary px-2 py-1 text-[11px] font-semibold text-primary">
                            Selected
                          </span>
                        )}
                      </div>
                    ...
                    <button
                      ...
                      title="Remove"
                      className="w-7 h-7 shrink-0 rounded-full flex items-center justify-center text-on-surface-variant opacity-0 group-hover:opacity-100 hover:bg-error-container hover:text-error transition-all"
                    >
